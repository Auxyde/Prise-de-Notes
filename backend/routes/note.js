...
router.post("/", Notes.insert_note); // Ajout d'une note
router.patch("/:val", jwtCheck, Notes.update_note); // Mise à jour d’une note (title et/ou content )
router.delete("/:id", Notes.delete_note); //Suppression de la note à partir de son _id
/***************************************/
router.use((req, res) => {
    res.status(404);
    res.json({
